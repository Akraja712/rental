...
if (isset($_POST['mobile']) && isset($_POST['to_mobile']) && isset($_POST['transfer_amount'])) {
    ...
    $senderQuery = "SELECT * FROM users WHERE mobile = '$toMobile'";
    $db->sql($senderQuery);
    $sender = $db->getResult();
    ...
    $updateSender = "UPDATE users SET recharge = $newSenderBalance WHERE mobile = '$toMobile'";
    $db->sql($updateSender);

    $updateReceiver = "UPDATE users SET recharge = $newReceiverBalance WHERE mobile = '$mobile'";
