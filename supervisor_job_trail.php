...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $_SESSION['submission_count_3'][$plan_id] = 0; // Reset count for a new session
...
if (!isset($_SESSION['submission_count_3'][$plan_id])) {
    $_SESSION['submission_count_3'][$plan_id] = 0;
}
...
if (isset($_POST['btnNext'])) {
    ...
    if (empty($qty) || $qty < 1) {
      ...
      if (!array_filter($errors)) {
        $_SESSION['submission_count_3'][$plan_id] += 1; // Increment submission count

        // Process the submission if max count reached
        if ($_SESSION['submission_count_3'][$plan_id] >= $max_submission_count_3) {
            // Reset the submission count for this plan_id
            $_SESSION['submission_count_3'][$plan_id] = 0; // Reset count for the next round
...
$claim_button_enabled = $_SESSION['submission_count_3'][$plan_id] >= $max_submission_count_3;

// Display the submission count
$display_count = $_SESSION['submission_count_3'][$plan_id];
?>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="product-name-box">Supervisor Job</div>
                    ...
                    <div class="mb-3">
                        <h5>Submission Count: <span class="highlighted-value"><?php echo $display_count; ?>/<?php echo htmlspecialchars($max_submission_count_3); ?></span></h5>
                    </div>
